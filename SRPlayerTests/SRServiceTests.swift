...
final class SRServiceTests: XCTestCase {
    func testFetchChannels() async throws {
        ...
        await mock.setModelResponse(to: .init(channels:
            .init(channel: [
                .init(id: 1, name: "Andys channel", image: image?.absoluteString, color: "ff0000", liveAudio: .init(url: URL(string: "http://sverigesradio.se/topsy/direkt/srapi/132.mp3"))),
            ])
        ))
        ...
        XCTAssertEqual(try unwrap(channels.first?.liveAudio?.url?.absoluteString), "http://sverigesradio.se/topsy/direkt/srapi/132.mp3")
    }
